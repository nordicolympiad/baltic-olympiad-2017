...
public class RandomTreeGenerator {
	...
	// creates a path [0,1,...,pathLength-1], and then attaches random leaves
	ArrayList<Integer> randomDeepTreeSequence(int pathLength, int numLeaves) {
		ArrayList<Integer> T = new ArrayList<Integer> ();
		
		int n = pathLength + numLeaves;
		
		// create main path.
		for(int i = 0; i < pathLength-1; ++i) {T.add(i);}
		
		// add leaves
		for(int i = pathLength; i < n; ++i) T.add(rn.nextInt(i+1));
	...
	ArrayList<ArrayList<Integer> > randomRootedDeepTree(int pathLength, int numLeaves) {
		return sequenceToTree(randomDeepTreeSequence(pathLength, numLeaves));
	}
	...
	ArrayList<ArrayList<Integer> > randomRootedDeepTreeNoParent(int pathLength, int numLeaves) {
		return sequenceToTreeNoParent(randomDeepTreeSequence(pathLength, numLeaves));
	}

	public static void main(String[] args) {
		System.out.println((new RandomTreeGenerator ()).randomRootedDeepTreeNoParent(10,40));
	}
